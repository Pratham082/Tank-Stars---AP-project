...
public class GameScreen_2 extends ScreenAdapter {
    ...
    public GameScreen_2(TankWars game) {
        this.game = game;
    }
    ...
    public void render(float delta) {
        ...
        range.setPosition(game.posTank2+50, 144-normalizeCOM+60);
        range.setOrigin(0, 0);
        range.setSize(initialVelocity*2,initialVelocity*2);
        range.setRotation(-cannonAngle+45);
        if(cannonAngle>-90){
        ...
        tank2.setPosition(game.posTank2,144 - normalizeCOM);
        ...
        tank2.draw(batch);
        range.draw(batch);
        if(game.healthTank1==0){
            game.setScreen(new WonScreen(game,2));
        }
        if(missileY<144){
            missileVisible = 0;
            float temp = Math.abs(game.posTank1 - missileX);
            if(temp<100){
                game.healthTank1 -= 100;
                game.healthTank1 += temp;
                game.posTank1 += temp;
            }
        }
        if(missileVisible==1) {
